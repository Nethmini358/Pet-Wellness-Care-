...
class vaccine_activity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.vaccine_activity) // connects to complete_activity.xml

        // Find button by its ID
        val btn_switch1 : Button = findViewById(R.id.btn_switch2)
        val btnAddVaccine : Button = findViewById(R.id.btnAddVaccine)

        // When button is clicked → open login
        btn_switch1.setOnClickListener {
            val intent = Intent(this, complete_activity::class.java)
            startActivity(intent)
        }
        // When button is clicked → open login
        btnAddVaccine.setOnClickListener {
            //Navigate to login
            val intent = Intent(this, addvaccine_activity::class.java)
            startActivity(intent)
        }
